...
export default function SignInModal() {
    ...
    // google signin using firebasee authentication
    const handleGoogleLogin = async () => {
        try {
            const result = await signInWithPopup(auth, googleProvider);
            const user = result.user;
            console.log("User signed in:", user);
            //  can redirect or store user info now
        } catch (error) {
    ...
    return (
        <>
            {isModalOpen && (<div
                ...
                className="fixed inset-0 bg-black/50 bg-opacity-20
                ...
                <div className="bg-white rounded-2xl p-6 w-[50%] max-w-full shadow-lg overflow-y-auto">
                    <div className="flex justify-between items-center mb-4 ">
                        ...
                        <button
                            onClick={handleClose}
                            className="text-2xl cursor-pointer">&times;</button>
                        {/* heading */}
